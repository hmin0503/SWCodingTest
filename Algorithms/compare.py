...
for testcase in range(s):
    eat = list()
    max_eat = -1
    # 1. 모든 물고기 복제
    temp = copy.deepcopy(graph)
    print("shark", shark)
    print("initial fish", testcase+1)
    for row in temp[:]: print(*row)

    # 2. 물고기 이동
    temp = move_fish()
    print("move fish", testcase + 1)
    for row in temp[:]: print(*row)
    print("fish smell", testcase + 1)
    for row in smell: print(*row)
    print()
    # 3. 상어이동 - 백트래킹
